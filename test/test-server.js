...
describe('Recipes', function () {
  ...
  it('Should Return a JSON for the Recipe', function () {
    return chai.request(app)
      .get('/recipes')
      .then(function (res) {
        expect(res).to.have.status(200);
  ...
  //Test Post for Recipes
  it('Should create a new entry for Recipes', function () {
    const newItem = { name: 'Chocolate Milk', ingredients: ['Milk', 'Sugar', 'Cocoa'] };
    return chai.request(app)
      .post('/recipes')
      .send(newItem)
    ...
    then(function (res) {
      ...
      expect(res.body.ingredients).to.be.equal(newItem.ingredients);
      expect(res).to.be.json;
      expect(res.body).to.be.a('array');
  ...
  //Test Put for Recipes 
  it('Should update an existing recipe', function () {

    const updateId = { name: 'foo', ingredients: '1 cup bar' };

    return chai.request(app)
      .get('/recipes')
      .then(function (res) {
        updateId.id = res.body[0].id;
        console.log('HELLO -->', updateId.id);
        return chai.request(app)
          .put(`/recipes/${updateId.id}`)
          .send(updateId)
      })

      .then(function (res) {
        console.log('HELLO AGAIN -->', res.body)
  ...
  //Test Delete for Recipes
  it('Should delete an existing recipe', function () {
    const deleteId = {};
    return chai.request(app)
      .get('/recipes')
      .then(function (res) {
        deleteId.id = res.body[0].id
        return chai.request(app)
          .delete(`/recipes/${deleteId.id}`)
